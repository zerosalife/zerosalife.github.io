...
UnityTestTools and duplicated test scripts.

To share code between projects I use symlinks to link the files from
...
One caveat I've noticed is that the symlinked files show up in my git
version control as a blob of the path text instead of the actual code
of the file.
...
Google Docs is great for mock-ups.  Unfortunately Google Draw doesn't
support animation or multiple pages.  Luckily the same suite of tools
is available in Google's PowerPoint app.  If you don't like the
PowerPoint interface, sketch your mock-ups in Draw and copy them over
to PowerPoint.
...
philosophy that space is cheap feel, so free to experiment and keep
everything because you never know what will influence the final
design.

** What is or isn't a game?
Summarizing my thoughts on @tgdfweb's eloquent definition of games as
play
