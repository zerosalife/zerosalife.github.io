...
faint, the actual RGB values will approach, but not reach 100%.  When
we apply a tint to the blobs' sprites we are essentially multiplying
each of the RGB values by our desired color's value.  The resulting
value will approach the full /intensity/ of the color as our raw blob
approaches 100% white, if that makes sense.  So the result is a faint
blob of any desired color we want.

The colored blobs overlap each other, generating interesting patterns
of lighter and darker mixtures of the two colors.

*** Blobonia
Using my favorite image editing program, I made a 256 x 256 =png= blob
sprite.  Using the airbrush tool, I plopped a blob in the middle of
the canvas.
...
I created a fresh 2D project and imported the blob sprite.  I dragged
it into the scene to create a =GameObject= with the blob sprite.

I added a =Sorting Layer= named =Blobs= and set the sprite's sorting
layer to =Blobs=.  While I was at it, I created a sorting layer named
=BG= behind the =Blobs= layer for the background.

I named =GameObject= =Blob= and created a prefab from it.

Blob prefab in hand, I still needed a couple more pieces to prepare
the scene.

*** Backdrop

I created an black =Background= UI object and set the =X scale= and =Y
...
It would be nice to control the local density of the blobs in regions,
so the map is a little less uniform.  But I think the effect we've
achieved works as a background.
